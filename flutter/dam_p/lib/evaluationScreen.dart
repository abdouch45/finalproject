...
import '/components/myInput.dart';
...
// add new viva //
  createNewViva() async {
    ...
    if(_projectName.text.isNotEmpty && _year.text.isNotEmpty &&_supName.text.isNotEmpty &&_preName.text.isNotEmpty &&_exaName.text.isNotEmpty &&_s1Name.text.isNotEmpty &&_supMark.text.isNotEmpty && _preMark.text.isNotEmpty && _exaMark.text.isNotEmpty ){
      ...
      http.Response response = await Viva.create(data).timeout(const Duration(seconds: 20), onTimeout: () {
        setState(() {
          isloading=false;
        });
        print('test');
...
//success modal
  ...
  Widget build(BuildContext context) {
    return isloading ? Scaffold(
      ...
      body: Container(
        child: Center(
          child:LoadingAnimationWidget.twoRotatingArc(color: Colors.white, size: 70) ,
        ),
      ),
            ...
            children: [
             ...
             if(_preMark.text.isNotEmpty){
              ...
              Row(
                ...
                _preMark.text.isEmpty ||  _exaMark.text.isEmpty || _supMark.text.isEmpty? Text('',style: TextStyle(fontSize: 20),):Text('${(sm*0.3+em*0.4+pm*0.3).toStringAsFixed(2)}',style: TextStyle(fontSize: 20)),
                //  SizedBox(width: 2,)
                ],
